...
\begin{marker}
LED模块的参考代码位于\Colorbox{lightgrey}{\lstinline{examples/peripherals/rmt/led\_strip}}文件夹。
\end{marker}
...
注册应用程序事件是程序生命周期中触发的第一个事件，此示例使用Profile A GATT事件句柄在注册时配置广告参数。此示例提供了使用标准蓝牙核心规范广告参数或自定义原始缓冲区的选项。可以通过\Colorbox{lightgrey}{\lstinline{CONFIG_SET_RAW_ADV_DATA}}定义来选择此选项。原始广告数据可用于实现iBeacon、Eddystone或其他专有和自定义帧类型，如用于室内定位服务的帧类型，这些帧类型与标准规范不同。

用于配置标准蓝牙规范广告参数的函数是\Colorbox{lightgrey}{\lstinline{esp_ble_gap_config_adv_data()}}，它接受一个指向\Colorbox{lightgrey}{\lstinline{esp_ble_adv_data_t}}结构的指针。广告数据的\Colorbox{lightgrey}{\lstinline{esp_ble_adv_data_t}}数据结构定义如下：
...
最小和最大从设备首选连接间隔以1.25毫秒为单位设置。在此示例中，最小从设备首选连接间隔定义为\Colorbox{lightgrey}{\lstinline{0x0006 * 1.25毫秒 = 7.5毫秒}}，最大从设备首选连接间隔初始化为\Colorbox{lightgrey}{\lstinline{0x0010 * 1.25毫秒 = 20毫秒}}。
...
服务被定义为一个具有16位UUID长度的主服务。服务ID使用实例ID = 0和由\Colorbox{lightgrey}{\lstinline{GATTS_SERVICE_UUID_TEST_A}}定义的UUID进行初始化。
...
首先，由BLE协议栈生成的服务句柄存储在配置文件表中，稍后应用层将使用它来引用此服务。然后，设置特征的UUID及其UUID长度。特征UUID的长度再次为16位。使用先前生成的服务句柄，通过\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_start_service()}}函数启动服务。触发了\Colorbox{lightgrey}{\lstinline{ESP_GATTS_START_EVT}}事件，该事件用于打印信息。通过\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_add_char()}}函数结合特征的权限和属性将特征添加到服务中。在这个示例中，两个配置文件中的特征如下设置：
...
事件返回的属性句柄存储在配置文件表中，同时也设置了特征描述符的长度和UUID。使用\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_get_attr_value()}}函数读取特征的长度和值，然后出于信息目的打印它们。最后，使用\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_add_char_descr()}}函数添加特征描述符。使用的参数包括服务句柄、描述符UUID、写和读权限、一个初始值以及自动响应设置。特征描述符的初始值可以是一个NULL指针，自动响应参数也设置为NULL，这意味着需要响应的请求必须手动回复。
...
\Colorbox{lightgrey}{\lstinline{esp_ble_gap_update_conn_params()}} 函数出发了一个 GAP 事件 \Colorbox{lightgrey}{\lstinline{ESP_GAP_BLE_UPDATE_CONN_PARAMS_EVT}}，这个函数用来打印连接信息：
...
既然服务和特征都已经创建并启动了，那么程序需要读取和写入事件。读取操作被表示为 \Colorbox{lightgrey}{\lstinline{ESP_GATTS_READ_EVT}} 事件，这个事件有以下参数：
...
在这个示例中，使用由事件给定的相同句柄，构造了一个包含虚拟数据的响应，并使用\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_send_response()}}函数将其发送回主机。除了响应外，GATT接口、连接ID和传输ID也作为参数包含在\Colorbox{lightgrey}{\lstinline{esp_ble_gatts_send_response()}}函数中。如果在创建特征或描述符时，自动响应字节被设置为\Colorbox{lightgrey}{\lstinline{NULL}}，则需要此函数。
