...
安装程序允许将程序下载到现有的 ESP-IDF 目录。推荐将 ESP-IDF 下载到 \lstinline{%userprofile%\Desktop\esp-idf} 目录下，其中 \lstinline{%userprofile%} 代表家目录。

\section{启动ESP-IDF环境}

安装结束时，如果勾选了 \lstinline{Run ESP-IDF PowerShell Environment} 或 \lstinline{Run ESP-IDF Command Prompt (cmd.exe)}，安装程序会在选定的提示符窗口启动 ESP-IDF。

\lstinline{Run ESP-IDF PowerShell Environment}：

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{esp-idf-installer-screenshot-powershell.png}
  \caption{PowerShell}
\end{figure}

\chapter{创建工程}

现在，可以准备开发 ESP32 应用程序了。可以从 ESP-IDF 中 examples 目录下的 \lstinline{get-started/hello_world} 工程开始。

\begin{marker}
ESP-IDF 编译系统不支持 ESP-IDF 路径或其工程路径中带有空格。
\end{marker}

将 \lstinline{get-started/hello_world} 工程复制至本地的 \lstinline{~/esp} 目录下：
...
请进入 \lstinline{hello_world} 目录，设置 ESP32-C3 为目标芯片，然后运行工程配置工具 menuconfig 。
...
打开一个新工程后，应首先使用 \lstinline{idf.py set-target esp32c3} 设置“目标”芯片。注意，此操作将清除并初始化项目之前的编译和配置（如有）。也可以直接将“目标”配置为环境变量（此时可跳过该步骤）。

正确操作上述步骤后，系统将显示以下菜单：

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{project-configuration.png}
  \caption{配置界面示意图}
\end{figure}

可以通过此菜单设置项目的具体变量，包括 Wi-Fi 网络名称、密码和处理器速度等。\lstinline{hello_world} 示例项目会以默认配置运行，因此在这一项目中，可以跳过使用 menuconfig 进行项目配置这一步骤。
...
如果一切正常，编译完成后将生成 \lstinline{.bin} 文件。
...
\begin{marker}
勾选 flash 选项将自动编译并烧录工程，因此无需再运行 \lstinline{idf.py build}。]
\end{marker}
...
如果一切顺利，烧录完成后，开发板将会复位，应用程序 \lstinline{hello_world} 开始运行。
